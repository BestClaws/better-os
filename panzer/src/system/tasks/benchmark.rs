...
pub async fn run(
    ...
    match pixel_format {
        PixelFormat::Gray4 => run_luma4(display, buffer, width, height).await,
        PixelFormat::Rgb565 => run_rgb565(display, buffer, width, height).await,
    }
}
...
#[inline(always)]
fn run_case_luma4<F>(buffer: &mut [u8], width: u16, height: u16, mut f: F) -> Duration
where
    F: FnMut(&mut Luma4Rasterizer<'_>),
{
    buffer.fill(0);
...
#[inline(always)]
fn run_case_rgb565<F>(buffer: &mut [u8], width: u16, height: u16, mut f: F) -> Duration
where
...
async fn run_luma4(
    ...
    if width == 0 || height == 0 {
        info!("Luma4 benchmark skipped: zero dimension framebuffer");
        return;
    }

    info!("Running Luma4 rasterizer benchmarks");
    ...
    let fill_hspan = run_case_luma4(buffer, width, height, |rasterizer| {
    ...
    let blend_hspan = run_case_luma4(buffer, width, height, |rasterizer| {
        for y in 0..height_usize {
    ...
    let blend_color_hspan = run_case_luma4(buffer, width, height, |rasterizer| {
        for y in 0..height_usize {
    ...
    let fill_vspan = run_case_luma4(buffer, width, height, |rasterizer| {
    ...
    let blend_vspan = run_case_luma4(buffer, width, height, |rasterizer| {
        for x in 0..width_usize {
    ...
    let blend_color_vspan = run_case_luma4(buffer, width, height, |rasterizer| {
        for x in 0..width_usize {
    ...
    info!("Luma4 rasterizer benchmarks complete");
}
...
async fn run_rgb565(
    ...
    let fill_hspan = run_case_rgb565(buffer, width, height, |rasterizer| {
        for y in 0..height_usize {
            rasterizer.fill_solid_hspan(y as u16, 0, solid_color, width);
        }
    });
    ...
    let blend_hspan = run_case_rgb565(buffer, width, height, |rasterizer| {
        for y in 0..height_usize {
            rasterizer.blend_solid_hspan(y as u16, 0, blend_color, &coverage_row);
        }
    });
    ...
    let blend_color_hspan = run_case_rgb565(buffer, width, height, |rasterizer| {
        for y in 0..height_usize {
            rasterizer.blend_color_hspan(y as u16, 0, &color_row, &coverage_row);
        }
    });
    ...
    let fill_vspan = run_case_rgb565(buffer, width, height, |rasterizer| {
        for x in 0..width_usize {
            rasterizer.fill_solid_vspan(x as u16, 0, solid_color, height);
        }
    });
    ...
    let blend_vspan = run_case_rgb565(buffer, width, height, |rasterizer| {
        for x in 0..width_usize {
            rasterizer.blend_solid_vspan(x as u16, 0, blend_color, &coverage_col);
        }
    });
    ...
    let blend_color_vspan = run_case_rgb565(buffer, width, height, |rasterizer| {
        for x in 0..width_usize {
            rasterizer.blend_color_vspan(x as u16, 0, &color_col, &coverage_col);
        }
    });
