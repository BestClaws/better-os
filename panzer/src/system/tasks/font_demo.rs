...
pub fn run_font_demo(frame_buffer: &mut [u8], width: u16, height: u16, frame_counter: u32, pixel_format: PixelFormat) {
    match pixel_format {
        PixelFormat::Gray4 => run_font_demo_luma4(frame_buffer, width, height, frame_counter),
        PixelFormat::Rgb565 => run_font_demo_rgb565(frame_buffer, width, height, frame_counter),
    }
}

fn run_font_demo_luma4(frame_buffer: &mut [u8], width: u16, height: u16, frame_counter: u32) {
    ...
    info!("=== Luma4 Font Rendering Demo (Frame {}) ===", frame_counter);
...
fn run_font_demo_rgb565(frame_buffer: &mut [u8], width: u16, height: u16, frame_counter: u32) {
    let cache_start = Instant::now();
    let font = Font::builder()
        .data(FONT_DATA_PIXEL)
        .size(8.0)
