...
use crate::libs::hps::types::{
    HttpMethod, HttpResponse, MAX_BODY_SIZE, MAX_HEADERS_SIZE, MAX_URI_SIZE,
};
...
static HTTP_REQUEST_CHANNEL: Channel<CriticalSectionRawMutex, HpsRequest, 2> = Channel::new();
...
pub fn http_response_receiver() -> Receiver<'static, CriticalSectionRawMutex, HttpServiceResponse, 2>
{
...
pub(crate) async fn http_service() {
    ...
    loop {
        ...
        info!("HTTP service: Forwarding {} request", hps_request.method);
