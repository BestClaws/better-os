    ...
    RST: OutputPin + Send,
{
    ...
    async fn paint_screen(&mut self, color: u16) {
        ...
        let mut buffer = vec![color; CHUNK_SIZE / 2];
        ...
        for y in (0..self.height).step_by(CHUNK_HEIGHT as usize) {
            ...
            let len = (self.width * height) as usize * BYTES_PER_PIXEL;
            let chunk = &buffer_bytes[0..len];

            if let Err(_) = self.send_pixels(chunk).await {
                error!("Failed to send pixels");
                return;
            }
        }
    }
    ...
    async fn draw_gray4(&mut self, _buffer: &[u8], _scale: u32) {
        // Not implemented as requested
    }

    async fn draw_gray4_region(&mut self, _buffer: &[u8], _region: Rectangle, _scale: u32) {
        // Not implemented as requested
    }

    async fn draw(&mut self, _buffer: &[u8], _scale: u32) {
        // Not implemented as requested
    }
