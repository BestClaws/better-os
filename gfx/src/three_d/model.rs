...
fn decode_image_from_view(
    ...
    views: &[BufferView],
    binary: &[u8],
) -> Result<(u32, u32, Vec<Color>, Option<String>), ModelError> {
    let view = views
...
fn decode_png(data: &[u8]) -> Result<(u32, u32, Vec<Color>), ModelError> {
...
fn decode_jpeg(data: &[u8]) -> Result<(u32, u32, Vec<Color>), ModelError> {
    ...
    for chunk in pixels.chunks_exact(4) {
        output.push(Color::rgba(chunk[0], chunk[1], chunk[2], chunk[3]));
    }
