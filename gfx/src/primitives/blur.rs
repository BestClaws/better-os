...
pub fn draw_blur<R: Rasterizer>(rast: &mut R, dsc: &BlurDsc, area: &Area) {
    ...
    let expected_len = total_pixels * core::mem::size_of::<Color>();
    ...
    let buffer = unsafe {
        core::slice::from_raw_parts_mut(buf_bytes.as_mut_ptr() as *mut Color, total_pixels)
    };
...
fn blur_pixel(
    ...
    buffer[idx] = Color::rgba(r, g, b, color.a());
}
...
fn copy_pixel(buffer: &mut [Color], width: usize, src_x: i32, dst_x: i32, y: i32) {
    ...
    buffer[dst_idx] = Color::rgba(src.r(), src.g(), src.b(), dst_alpha);
}
