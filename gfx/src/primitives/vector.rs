...
impl LinearGradient {
    pub fn color_at(&self, point: FPoint) -> (Color, Opa) {
...
impl VectorFill {
    pub fn solid(color: Color, opa: Opa) -> Self {
        Self {
...
impl VectorStroke {
    pub fn new(width: f32, color: Color, opa: Opa) -> Self {
        Self {
            width,
            color: Some(color),
            opa,
...
fn render_fill<R: Rasterizer>(rast: &mut R, fill: &VectorFill, flattened: &Flattened) {
    ...
    let mut color_row = matches!(fill.kind, FillKind::LinearGradient(_))
        .then(|| vec![Color::TRANSPARENT; span_width]);
...
fn render_stroke<R: Rasterizer>(rast: &mut R, stroke: &VectorStroke, flattened: &Flattened) {
    ...
    let mut color_row = stroke
        .gradient
        .as_ref()
        .map(|_| vec![Color::TRANSPARENT; span_width]);
    ...
    for y in min_y..=max_y {
        ...
        for x in min_x..=max_x {
            ...
            let (base_color, grad_opa) = if let Some(grad) = &stroke.gradient {
                ...
                (stroke.color.unwrap_or(Color::WHITE), OPA_COVER)
            };
        ...
        if let Some(colors) = color_row.as_ref() {
            ...
            let solid_color = stroke.color.unwrap_or(Color::WHITE);
